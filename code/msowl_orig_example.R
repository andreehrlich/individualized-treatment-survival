...
source("./models/msowl/R/itr.R")
source("./models/msowl/R/V_d.R")
source("./models/msowl/R/wsvm_solve.R")
source("./models/msowl/R/zeta_h_t.R")
source("./models/msowl/R/zeta_t.R")


options(error=traceback) 

# The artificial dataset `example_data.csv` (included in this repository) 
# contains observations from an illness-death process without recovery. 
# The dataset can be obtained as follows
library(foreign)
data <- read.csv("./models/msowl/data/example_data.csv")
print("DATA")
head(data)
...
# Estimation of an optimal individual treatment rule for prolonging the time spent in 
# State 2 based on a linear decision function 
# and considering the process up to time τ = 3 can be performed as follows:
...
res$opt = V_d(data=data, w=c(1, 1, 0), tau=3, dec.fun=fit$fit, feat=c("z1", "z2"), SE = TRUE, )
...
res$pos = V_d(data=data, w=c(1, 1, 0), tau=3, dec.fun=1, feat=c("z1", "z2"), SE = TRUE)
print(paste("Value of Treatment 1:", res$pos[1], "+/-", res$pos[2]))

# Estimation of the the value function of the fixed rule that assigns treatment -1 to everyone, along with its standard error, can be performed as follows:
res$neg = V_d(data=data, w=c(1, 1, 0), tau=3, dec.fun=-1, feat=c("z1", "z2"), SE = TRUE)
